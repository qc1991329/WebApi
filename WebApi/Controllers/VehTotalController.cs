...
namespace WebApi.Controllers
{
    public class VehTotalController : ApiController
    {
        public ResultObject getVehTotal() {
            ...
            string queryString = "select at_province,total from (select t.at_province,sum(t.at_total) as total from AREA_VEHICLE_TOTAL t where t.at_year = 2014 and t.at_province is not null  group by t.at_province order by total DESC) where rownum<=10";
